...
<PageContent>
    ...
    <p>Crea un archivo llamado <strong><code>config/doctrine.local.php</code></strong>  y agrega el siguiente código:</p>
...
<p>Si estás utilizando una base de datos que no sea MySQL, ajusta el valor de 'driver' según corresponda.</p>


<hr>

    <h1>Permisos de Escritura para la Carpeta "data"</h1>
    <p>Para asegurar el correcto funcionamiento de la aplicación, es importante otorgar permisos de escritura a la carpeta "data". A continuación, se detallan los pasos para hacerlo en diferentes sistemas operativos:</p>
    
    <h2>Linux:</h2>
    <p>En sistemas Linux, puedes usar el comando <code>chmod</code> para cambiar los permisos de la carpeta "data" y otorgar permisos de escritura. Abre tu terminal y ejecuta el siguiente comando:</p>
    <pre><code>chmod a+w data</code></pre>
    <p>Esto permitirá que cualquier usuario en el sistema pueda escribir en la carpeta "data". </p>

    <h2>Windows:</h2>
    <p>En sistemas Windows, puedes cambiar los permisos de la carpeta "data" a través de la interfaz gráfica del explorador de archivos:</p>
    <ol>
        ...
        <li>Selecciona el usuario "Todos" en la lista de nombres de grupos o usuarios y marca la casilla "Control total" en la columna "Permitir".</li>
    ...
    <h2>macOS:</h2>
    <p>En macOS, puedes usar el comando <code>chmod</code> de manera similar a como se hace en Linux. Abre la Terminal y ejecuta el siguiente comando:</p>
    <pre><code>chmod a+w data</code></pre>
    <p>Esto permitirá que cualquier usuario en el sistema pueda escribir en la carpeta "data". </p>

    <p>
        <strong>Recuerda en entornos de producción aplicar las mejores prácticas de seguridad y otorgar permisos solo a los grupos y usuarios relevantes.</strong>
    </p>

<hr>

<h1>Crear Entidad Author (Autor)</h1>
<p>Para definir una entidad Doctrine que represente un autor, crea un nuevo archivo <code>Author.php</code> en la siguiente ubicación: <code>modules/AppModule/src/Entities/Author.php</code> y agrega el siguiente código:</p>
...
<p>La clase <code>Author</code> es una entidad Doctrine que utiliza anotaciones para definir sus columnas en la base de datos. Esta clase extiende la clase <code>AbstractEntityModel</code>, la cual agrega las propiedades <code>id</code> (clave primaria), <code>created</code> y <code>updated</code>.</p>

<p>Los métodos <code>get</code> se utilizan para generar los campos tipo objeto GraphQL. Para que funcionen correctamente, estos métodos deben ser públicos y tener un tipo de retorno asignado.</p>

<p>Los métodos <code>set</code> se utilizan para generar los tipos de entrada de GraphQL. Deben ser públicos y tener asignado un tipo requerido.</p>

<p>Para generar el código SQL necesario para actualizar la base de datos, ejecuta el siguiente comando en la terminal:</p>
...
<hr>
    ...
    <p>Para registrar un campo GraphQL que permita la creación de autores, es necesario crear la clase <code>AppModule</code> para que extienda la clase <code>GPDCore\Library\AbstractModule</code> e implemente los siguientes métodos:</p>
...
<CodeBlockOpenChar />
    // Otros métodos y propiedades de la clase AppModule
...
<CodeBlockEndChar />
    ...
    <p>Con estos cambios, la clase <code>AppModule</code> estará completamente configurada para registrar un campo GraphQL que permita la creación de autores en tu aplicación PHP.</p>

    <p>En el array devuelto por el método <code>getMutationFields</code>, se agrega un campo GraphQL. 
        La clave de este campo corresponde al nombre que se utilizará en las consultas GraphQL. 
        El valor asociado a esta clave es un objeto GraphQL creado mediante la clase <code>GPDFieldFactory</code> utilizando el método <code>buildFieldCreate</code>.</p>

	


            <h1>Crear un Autor mediante Consulta GraphQL</h1>
            <p>Para probar la creación de un autor, la URL de la API depende de la configuración del servidor y generalmente apunta al archivo principal en la ruta <code>public/index.php/api</code>.</p>
        
            <p>Por ejemplo, en un servidor Apache: <code>http://localhost/my-project/public/index.php/api</code></p>
        
            <p>También puedes iniciar un servidor PHP desde el directorio de la aplicación con el siguiente comando:</p>
            <pre><code>php -S localhost:8000 public/index.php</code></pre>
        
            <p>De esta manera, la URL sería: <code>http://localhost:8080/api</code></p>
        
            <p>Para realizar la prueba, puedes utilizar una extensión de GraphQL para tu navegador o Postman.</p>
        
            <p>La consulta GraphQL a ejecutar es la siguiente:</p>
        
            <CodeBlock> mutation <CodeBlockOpenChar />
        ...
        <CodeBlockEndChar /></CodeBlock>
            ...
            <p>Con la extensión GraphQL Playground para Chrome, puedes ejecutar la consulta y visualizar los resultados directamente en el navegador.</p>
        
            <p>Al ejecutar la consulta, se guardará un nuevo registro en la base de datos. Los campos <code>id</code>, <code>created</code> y <code>updated</code> son generados automáticamente.</p>
            ...
            <img src="/images/create-author.png" alt="" />
                ...
                <p>Para registrar un campo GraphQL que recupere el registro de un autor por su ID, actualiza el método <code>getQueryFields</code> en el archivo <code>modules/AppModule/src/AppModule.php</code> de la siguiente manera:</p>
            
<CodeBlock>function getQueryFields(): array
            <CodeBlockOpenChar />
                return [
            ...
            <CodeBlockEndChar /></CodeBlock>
                ...
                <p>Para probar la obtención del autor por ID, ejecuta el siguiente query en la extensión de Chrome GraphQL Playground:</p>
...
<CodeBlock>
            ...
            <p>
                ...
                <p>Para hacer más útil la consulta del autor, podemos agregar el nombre completo (<code>fullName</code>). Para ello, primero registramos un método <code>getFullName</code> en la clase <code>Author</code>:</p>
...
<CodeBlock>/**
            ...
            <CodeBlockEndChar /></CodeBlock>
                ...
                <p>Con esto, podemos recuperar el nombre completo de un autor desde la clase <code>Author</code> en PHP, y la propiedad <code>fullName</code> queda registrada en el esquema de GraphQL.</p>
                <p><img src="/images/get-author-fullname-schema.png" alt=""></p>
                <p>Para que funcione correctamente, es necesario agregar un resolver en la clase <code>Author</code> para <code>fullName</code>. Actualiza el método <code>getResolvers</code> en el archivo <code>modules/AppModule/src/AppModule.php</code> de la siguiente forma:</p>
            
<CodeBlock>function getResolvers(): array
            <CodeBlockOpenChar />
                return [
            ...
            <CodeBlockEndChar /></CodeBlock>
                ...
                <p>Es importante que la clave del array esté compuesta por el nombre de la clase seguido de dos veces dos puntos (<code>::</code>) y el nombre de la propiedad, en este caso, <code>Author::fullName</code>.</p>
            
                <p>Ahora puedes realizar la consulta solicitando el valor de <code>fullName</code>.</p>
        

                <p><img src="/images/get-author-fullname.png" alt=""></p>




                    <h1>Registrar un Campo GraphQL para Recuperar la Lista de Autores Registrados</h1>
                    <p>Para obtener la lista de autores registrados, actualiza el método <code>getQueryFields</code> en el archivo <code>modules/AppModule/src/AppModule.php</code> de la siguiente manera:</p>
    ...
    <CodeBlock>function getQueryFields(): array
                ...
                <CodeBlockEndChar />
                    ...
                    <p>Antes de realizar la prueba, puedes crear más autores utilizando la entrada <code>createAuthor</code> para poder visualizar mejor el resultado.</p>
                
                    <p>Para probar, ejecuta el siguiente query en la extensión GraphQL:</p>
                
    <CodeBlock>query <CodeBlockOpenChar />
                ...
                <p><img src="/images/authors-list.png" alt=""></p>
                    ...
                    <p>La consulta <code>getAuthorsList</code> ya incluye opciones para ordenar y filtrar. Por ejemplo, para filtrar los resultados por nombre, puedes ejecutar la siguiente consulta:</p>
...
<CodeBlock>
                ...
                <CodeBlockEndChar /></CodeBlock>
                    ...
                    <p><strong>NOTA:</strong> Puedes utilizar filtros y orden en la misma consulta.</p>
                        ...
                        <p>Antes de agregar la entrada para consultar la lista paginada, es necesario registrar los tipos <code>connection</code> y <code>Edge</code> para el tipo de dato que se va a consultar, en este caso <code>Author</code>.</p>
                    
                        <p>En la carpeta <code>modules/AppModule/src/Graphql</code>, crea el archivo <code>AuthorEdgeFactory.php</code> con el siguiente código:</p>
        ...
        <CodeBlock>&lt;?php
                    ...
                    <CodeBlockEndChar />
                        ...
                        <p>Esta clase extiende a <code>AbstractEdgeTypeServiceFactory</code>, que tiene los métodos necesarios para construir el objeto GraphQL <code>AuthorEdge</code>.</p>
                    
                        <p>La constante <code>NAME</code> corresponde al nombre del objeto GraphQL que se va a crear y no debe contener caracteres especiales ni espacios en blanco. La constante <code>DESCRIPTION</code> corresponde a la descripción del objeto GraphQL que se va a crear. La propiedad estática <code>$instance</code> es importante porque es donde se almacenará la instancia del objeto GraphQL una vez que se haya creado, de esta forma solo se creará un solo objeto de este tipo; las siguientes solicitudes para obtener el objeto retornarán el objeto creado previamente.</p>
                    
                        <p>Todos los objetos GraphQL deben ser registrados como servicios. Para crear y registrar como servicio el objeto <code>AuthorEdge</code>, actualiza el método <code>getServicesAndGQLTypes</code> de la clase <code>AppModule</code> en el archivo <code>modules/AppModule/src/AppModule.php</code> de la siguiente forma:</p>
                    
 <CodeBlock>function getServicesAndGQLTypes(): array
                    <CodeBlockOpenChar />
                        return [
                            &apos;invokables&apos; =&gt; [],
                            &apos;factories&apos; =&gt; [
                    ...
                    <CodeBlockEndChar /></CodeBlock>
                        ...
                        <p>En el array <code>factories</code>, se agrega la clave con el nombre del nuevo objeto (<code>AuthorEdgeFactory::NAME</code>), y como valor se agrega el retorno del método <code>AuthorEdgeFactory::getFactory</code> con los parámetros de contexto (<code>$this->context</code>) y la clase de la entidad relacionada (<code>Author::class</code>).</p>
                    
                        <p>Para el objeto <code>AuthorConnection</code>, se realiza un proceso similar:</p>
                    
                        <p>En la carpeta <code>modules/AppModule/src/Graphql</code>, crea el archivo <code>AuthorConnectionFactory.php</code> con el siguiente código:</p>
                    ...
                    <CodeBlockEndChar />
                        ...
                        <p>Las constantes <code>NAME</code>, <code>DESCRIPTION</code> y <code>$instance</code> tienen la misma funcionalidad que en <code>AuthorEdgeFactory</code>.</p>
                    ...
                    <CodeBlockEndChar /></CodeBlock>
                        ...
                        <p>Para este caso, a <code>AuthorConnectionFactory::getFactory</code> se le pasan las propiedades de contexto y el nombre del objeto Edge relacionado, en este caso <code>AuthorEdgeFactory::NAME</code> (<code>"AuthorEdge"</code>).</p>
                    
                        <p>Siempre es importante recordar que para importar otras clases es necesario agregar los namespaces al inicio del archivo. Por ejemplo, en el archivo <code>AppModule</code>, después de la línea <code>namespace AppModule;</code>, se agregan las líneas:</p>
                    
                        <CodeBlock>use AppModule\Graphql\AuthorConnectionFactory;
                    use AppModule\Graphql\AuthorEdgeFactory;</CodeBlock>
                    
                        <p>Una vez creados los tipos <code>AuthorEdge</code> y <code>AuthorConnection</code>, se puede actualizar el método <code>getQueryFields</code> de la clase <code>AppModule</code> para agregar la consulta paginada:</p>
                    ...
                    <p>recupera el objeto GraphQL <code>AuthorConnection</code> desde el administrador de servicios y la línea</p>
                    ...
                    <p><img src="/images/authors-list-pagination.png" alt=""></p>
                        <p>En este caso, se utiliza el input para recuperar las primeras <code>n</code> registros. El resultado contiene la siguiente información:</p>
                    
                        <ul>
                            <li><code>totalCount</code>: Total de elementos.</li>
                            <li><code>pageInfo</code>: Información de la página.</li>
                            <li><code>edges</code>: En la propiedad <code>nodes</code>, se encuentran los datos de los registros consultados.</li>
                        </ul>
...
<p>Crear archivo Post.php para la entidad en la siguiente ubicación modules/AppModule/src/Entities/Post.php</p>
<CodeBlock>
...
<p>
    Es necesario agregar <code>use GraphQL\Doctrine\Annotation as API;</code>  en la parte superior de la página para que se incluyan los archivos correspondientes 
</p>
...
<p>El método <code>ResolverFactory::createEntityResolver</code> recibe dos parámetros el primero es un objeto tipo EntityBuffer que se encarga de optimizar la consulta para obtener los datos de autores de todos los post relacionados en una sola consulta (ver n+1 problem)</p>
<p>
...
<p>En este caso, se utiliza <code>ResolverFactory::createCollectionResolver</code> porque se recupera una lista de elementos en lugar de solo uno. Es importante destacar que cada registro de autor recupera una lista de objetos Post.</p>
...
<p>Con todas estas configuraciones establecidas, ahora podemos utilizar la API para realizar una variedad de acciones con respecto a los posts: 
    visualizar, crear, actualizar y eliminar. </p>
